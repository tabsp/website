...
export const createPages: GatsbyNode["createPages"] = async ({
  ...
  const blogTag = path.resolve("src/templates/blog-tag.tsx")
  ...
  const tags = result.data!.allMarkdownRemark.group.map(
  ...
  // 创建标签页
  tags.forEach((tag: { name: string; slug: string; totalCount: number }) => {
    ...
    Array.from({ length: numPages }).forEach((_, i) => {
      ...
      const legacyPath =
        i === 0 ? `/tags/${legacySegment}` : `/tags/${legacySegment}/${i + 1}`
...
export const onCreateNode: GatsbyNode["onCreateNode"] = ({
  ...
  if (node.internal.type === `MarkdownRemark`) {
    ...
    const stats = readingTime((node.rawMarkdownBody as string) || ``)
